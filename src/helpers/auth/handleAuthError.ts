...
export function handleAuthError(
  res?: FetchResponse | { statusCode?: number; isBlocked?: boolean; message?: string; success?: boolean } | null
): boolean {
  if (!res) return false;
  ...
  const isBlocked =
    res.isBlocked ||
    res.statusCode === 403 ||
    (typeof res.message === "string" && res.message.toLowerCase().includes("blocked"));
